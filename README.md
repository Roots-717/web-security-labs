# Лабораторні роботи з безпеки вебзастосунків

У репозиторії містяться звіти з виконання практичних завдань
у навчальних середовищах OWASP WebGoat та OWASP Juice Shop.

## Перелік лабораторних робіт

1. [WebGoat — Hijack a Session](./WebGoat-Hijack-Session/)
2. [WebGoat — Insecure Direct Object References](./WebGoat-Insecure%20Direct%20Object%20References/)
3. [WebGoat — SQL Injection Intro](./WebGoat%20-%20SQL_intro/)
4. [OWASP Juice Shop](./Juice%20Shop/)
5. [CTF VulnHub — Deathnote](./CTF-DeathNote/)

## Використане програмне забезпечення

- OWASP WebGoat;
- OWASP Juice Shop;
- Burp Suite Community Edition;
- Docker;
- Kali Linux;
- Visual Studio Code.

